...
class TestUtil(NFSUtil):
    ...
    def _init_options(self):
        ...
        self.dbg_opgroup.add_option("--nomount", action="store_true", default=False, help=hmsg)
    ...
    def scan_options(self):
        ...
        if self.optfiles and self._fileopt:
            ...
            try:
                ...
                self.debug_level(c.NFSTEST_DEBUGLEVEL)
            except Exception, e:
                self.opts.error("Invalid verbose level <%s>: %s" % (opts.verbose, e))

            if len(opts.basename) == 0:
                self.logfile = "%s/%s.log" % (c.NFSTEST_LOG, self.get_name())
            ...
            if not self.server:
                ...
                self.server = urlopen('http://ip.42.pl/raw').read()
